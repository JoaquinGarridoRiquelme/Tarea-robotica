...
CORRECCIÓN: nombres de columnas actualizados para coincidir con el CSV:
  ps2_raw, ps5_raw, linear_adv_m, left_enc, right_enc
"""
...
def plot_all(rows, out_dir: str):
    ...
    ps2_raw         = extract(rows, "ps2_raw")        # CORREGIDO (era ps2_raw)
    ps5_raw         = extract(rows, "ps5_raw")        # CORREGIDO (era ps5_raw)
    left_enc        = extract(rows, "left_enc")       # CORREGIDO (era left_enc)
    right_enc       = extract(rows, "right_enc")      # CORREGIDO (era re_enc)
    linear_adv      = extract(rows, "linear_adv_m")   # CORREGIDO (era ds_m)
    ...
    ax5 = make_ax(2, 0, "Sensores diagonales (ps2 izq / ps5 der)")
    ax5.plot(t, ps2_raw, color="#2ecc71", lw=1.4, label="ps2 (diag izq)")
    ax5.plot(t, ps5_raw, color="#e67e22", lw=1.4, label="ps5 (diag der)")
